...
def register_routes():
    ...
    @server.routes.post("/director_plus/dasiwa/h3/forge/models")
    ...
    @server.routes.post("/director_plus/dasiwa/h3/forge/cancel")
    async def forge_cancel(request):
        body = await request.json()
        return web.json_response({"cancelled": cancel(body.get("request_id"))})

    @server.routes.post("/director_plus/dasiwa/h3/forge")
