...
// fields. Runs through /director_plus/dasiwa/h3/forge (director/h3_forge.py), outside the
// ComfyUI queue: the LLM writes, unloads, and only then is the workflow run.
// The Director exposes node.__directorPlusH3Forge for reading the timeline and
...
const SETTING_OLLAMA = "DirectorPlus.H3Forge.OllamaURL";
const SETTING_OPENAI = "DirectorPlus.H3Forge.OpenAIURL";
app.registerExtension({
  name: "DirectorPlus.H3Forge",
  settings: [
    { id: SETTING_OLLAMA, category: ["Director Plus", "H3 Forge", "Ollama address"], name: "Ollama address", type: "text", defaultValue: "", tooltip: "Leave empty for Ollama on this computer (http://127.0.0.1:11434). Set it to use Ollama on another machine." },
    { id: SETTING_OPENAI, category: ["Director Plus", "H3 Forge", "OpenAI-compatible server"], name: "OpenAI-compatible server address", type: "text", defaultValue: "", tooltip: "Optional: a llama.cpp server, llama-swap, LM Studio or koboldcpp, e.g. http://127.0.0.1:8080. Empty = off." },
  ],
});
...
const NO_MODELS = "No models found. Easiest fix: put a vision model folder (for example Qwen3-VL-8B-Instruct from Hugging Face) in ComfyUI/models/llm and reopen Forge. Or install Ollama and run: ollama pull qwen3-vl:8b. Other servers: Settings > Director Plus > H3 Forge.";

const STORE_KEY = "directorPlus.h3forge";
const briefs = new Map(); // node id -> last brief, for a reroll after closing
const HISTORY_KEY = "directorPlusH3ForgeHistory";
...
function saveForgeResult(node, result, brief) {
  ...
  node.__directorPlusH3Render?.(); // the Director Clear button must enable even when only drafts exist
  return entry;
}
...
function installStyles() {
  if (document.getElementById("dp-h3-forge-styles")) return;
  const style = document.createElement("style");
  style.id = "dp-h3-forge-styles";
  style.textContent = `
  .dp-h3-modebar .dp-h3-forge-btn,.dp-h3-forge-btn{background:rgba(151,91,255,.14)!important;color:#e6d9ff!important;border-color:rgba(177,128,255,.7)!important}
  .dp-h3-modebar .dp-h3-forge-btn:hover,.dp-h3-forge-btn:hover{box-shadow:0 0 10px rgba(151,91,255,.6)}
  .dp-forge-overlay{position:fixed;inset:0;z-index:10000;background:rgba(0,0,0,.55);display:flex;align-items:center;justify-content:center}
  .dp-forge{width:min(760px,94vw);max-height:90vh;overflow:auto;background:#111820;color:#e5eef4;border:1px solid #40515e;border-radius:8px;padding:14px;font:13px system-ui,sans-serif;display:flex;flex-direction:column;gap:10px;box-shadow:0 10px 40px rgba(0,0,0,.6)}
  .dp-forge h3{margin:0;font-size:15px;display:flex;justify-content:space-between;align-items:center}
  .dp-forge label{color:#9fb3c2;font-weight:600;font-size:12px}
  .dp-forge textarea,.dp-forge select,.dp-forge input[type=text]{width:100%;box-sizing:border-box;background:#0d1217;color:#e5eef4;border:1px solid #40515e;border-radius:4px;padding:7px;font:inherit}
  .dp-forge textarea{min-height:90px;resize:vertical}
  .dp-forge .row{display:grid;grid-template-columns:1fr 1fr;gap:10px}
  .dp-forge .field{display:flex;flex-direction:column;gap:4px}
  .dp-forge input[type=range]{width:100%}
  .dp-forge button{background:#202b35;color:#dbe7f0;border:1px solid #40515e;border-radius:4px;padding:6px 12px;cursor:pointer;font:inherit}
  .dp-forge button:hover{background:#2c3c49}
  .dp-forge button.primary{background:rgba(151,91,255,.3);border-color:rgba(177,128,255,.8);color:#fff;font-weight:600}
  .dp-forge button:disabled{opacity:.45;cursor:default}
  .dp-forge .actions{display:flex;gap:8px;justify-content:flex-end;align-items:center}
  .dp-forge .status{flex:1;color:#f3c67a;min-height:16px}
  .dp-forge .status.error{color:#ff8a8a}
  .dp-forge .muted{color:#8fa3b2;font-size:12px}
  .dp-forge .refs{display:flex;flex-direction:column;gap:6px}
  .dp-forge .ref{display:grid;grid-template-columns:48px 90px 130px 1fr;gap:8px;align-items:center}
  .dp-forge .ref img{width:48px;height:36px;object-fit:cover;border-radius:3px;background:#090d11}
  .dp-forge pre{white-space:pre-wrap;background:#0b1015;border:1px solid #344452;border-radius:4px;padding:8px;margin:0;max-height:320px;overflow:auto;font:12px/1.45 ui-monospace,monospace}
  .dp-forge .history{display:flex;flex-direction:column;gap:5px;border-top:1px solid #344452;padding-top:9px}
  .dp-forge .history-head{display:flex;align-items:center;justify-content:space-between;gap:8px}
  .dp-forge .history-head button{padding:3px 8px;font-size:11px}
  .dp-forge .history button{text-align:left;display:flex;flex-direction:column;gap:3px;min-width:0}
  .dp-forge .history button.selected{border-color:#b180ff;background:rgba(151,91,255,.18)}
  .dp-forge .history .excerpt{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:#9fb3c2;font-size:11px}
  `;
...
async function open(node) {
  const hook = node.__directorPlusH3Forge;
  if (!hook) return;
  installStyles();
  const mode = hook.mode();
  const prefs = remembered();

  const overlay = el("div", { className: "dp-forge-overlay" });
  const box = el("div", { className: "dp-forge" });
  overlay.append(box);
  // A run in flight, so Cancel and closing the pop-out can stop it.
  let running = null;
  const cancelRun = () => { if (running) { const id = running; running = null; api.fetchApi("/director_plus/dasiwa/h3/forge/cancel", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ request_id: id }) }).catch(() => {}); } };
  ...
  let levels = {};
  try {
    const res = await api.fetchApi("/director_plus/dasiwa/h3/forge/models", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ settings: forgeSettings() }) });
  ...
  genBtn.onclick = async () => {
    ...
    try {
      const res = await api.fetchApi("/director_plus/dasiwa/h3/forge", {
        method: "POST",
...
installStyles();
window.DirectorPlusH3Forge = { open, clearHistory: clearForgeHistory };
